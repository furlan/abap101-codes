*&---------------------------------------------------------------------*
*& Report zrefactor_v3b
*&---------------------------------------------------------------------*
*&
*&---------------------------------------------------------------------*
REPORT zrefactor_v3b LINE-SIZE 90.
...
CLASS test_purchase_order IMPLEMENTATION.
  ...
  METHOD add_valid_product.
    ...
    DATA items TYPE items_table.
    me->test_purchase_order->get_items( IMPORTING items_list = items ).

    DATA(item) = items[ 1 ]->get( ).
    cl_abap_unit_assert=>assert_equals( act = item exp = product_data ).
